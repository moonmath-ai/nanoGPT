...
# Architecture: blocks (n_layer x causal_cross_attn) with decreasing y slice lengths
# Progressive funnel via cross-attention: each block cross-attends to x_persistent with shrinking y
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.model = nn.ModuleDict(dict(
            ...
            blocks = nn.ModuleList([Transformer(config, attn_type='causal_cross_attn') for _ in range(config.n_layer)]),
    ...
    def forward(self, input, output_target=None, loss_last_only=False):
        ...
        # Transformer blocks (funnel: gradually truncate y to compress)
        # Each block: k,v from x_persistent (fixed), q from y (truncated x)
        x = self.model.blocks[0](x_persistent, y=x[:, -12*q_len:, :], rope_start_idx=0)  # (B, 12*q_len, n_embd)
        x = self.model.blocks[1](x_persistent, y=x[:, -8*q_len:, :], rope_start_idx=0)   # (B, 8*q_len, n_embd)
        x = self.model.blocks[2](x_persistent, y=x[:, -6*q_len:, :], rope_start_idx=0)   # (B, 6*q_len, n_embd)
        x = self.model.blocks[3](x_persistent, y=x[:, -4*q_len:, :], rope_start_idx=0)   # (B, 4*q_len, n_embd)
        x = self.model.blocks[4](x_persistent, y=x[:, -3*q_len:, :], rope_start_idx=0)   # (B, 3*q_len, n_embd)
        x = self.model.blocks[5](x_persistent, y=x[:, -2*q_len:, :], rope_start_idx=0)   # (B, 2*q_len, n_embd)
        x = self.model.blocks[6](x_persistent, y=x[:, -int(1.5*q_len):, :], rope_start_idx=0)  # (B, 1.5*q_len, n_embd)
        x = self.model.blocks[7](x_persistent, y=x[:, -q_len:, :], rope_start_idx=0)     # (B, q_len, n_embd)
