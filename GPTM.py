...
# Architecture: n_layer x full_self_attn (bidirectional)
# No compression: output width = input width
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.model = nn.ModuleDict(dict(
            ...
            blocks = nn.ModuleList([Transformer(config, attn_type='full_self_attn') for _ in range(config.n_layer)]),
    ...
    def forward(self, input, output_target=None, loss_last_only=False):
        ...
        Returns:
            logits: Output logits of shape (B, T, vocab_cardinality)
        ...
        # Transformer blocks (full self-attention, bidirectional)
        for block in self.model.blocks:
            x = block(x, rope_start_idx=0)  # (B, T, n_embd)

        # Output layer
        x = self.model.ln_o(x)  # (B, T, n_embd)
        logits = self.model.e2v(x)  # (B, T, vocab_cardinality)
        
        # Compute loss
        if output_target is not None:
            target = output_target  # (B, T)
            if loss_last_only:
                # Loss on last token only: (B, vocab_cardinality) vs (B,)
                loss = F.cross_entropy(logits[:, -1, :], target[:, -1])
            else:
                # Loss on all output tokens: (B*T, vocab_cardinality) vs (B*T,)
    ...
    def generate(self, input, max_new_tokens, temperature=1.0, top_k=None, max_input_len=None):
        ...
        Note: This is a bidirectional model (full_self_attn), so autoregressive
        generation may not produce coherent results. Use for experimentation only.
        ...
        for _ in range(max_new_tokens):
            ...
            logits, _ = self(input if max_input_len is None else input[:, -max_input_len:])  # (B, T, vocab_cardinality)
